...
// Hapus sambutan
if ($module=='sambutan' AND $act=='hapus'){
  $data=mysqli_fetch_array(mysqli_query($koneksi,"SELECT gambar FROM sambutan WHERE id_sambutan='$_GET[id]'"));
  if ($data['gambar']!=''){
     mysqli_query($koneksi,"DELETE FROM sambutan WHERE id_sambutan='$_GET[id]'");
...
// Hapus terpilih
elseif($module=='sambutan' AND $act=='hapussemua'){
if(isset($_POST['cek'])){
foreach($_POST['cek'] as $cek => $num){
$data=mysqli_fetch_array(mysqli_query($koneksi,"SELECT gambar FROM sambutan WHERE id_sambutan=$num"));
  if ($data['gambar']!=''){
     unlink("../../../img_teraskreasi/$data[gambar]");   
     unlink("../../../img_teraskreasi/small_$data[gambar]");  
} 
mysqli_query($koneksi,"DELETE FROM sambutan WHERE id_sambutan=$num");
  header('location:../../media.php?module='.$module.'&msg=delete');
}
} else {
  header('location:../../media.php?module='.$module.'&msg=delete');
...
// Input sambutan
elseif ($module=='sambutan' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
    ...
    echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG');
        window.location=('../../media.php?module=sambutan)</script>";
    }
    else{
    UploadPic($nama_file_unik);

    mysqli_query($koneksi,"INSERT INTO sambutan(judul,
                                    judul_seo,
                                    username,
                                    isi_sambutan,
                                    jam,
...
// Update sambutan
elseif ($module=='sambutan' AND $act=='update'){
  $lokasi_file    = $_FILES['fupload']['tmp_name'];
  $tipe_file      = $_FILES['fupload']['type'];
  $nama_file      = $_FILES['fupload']['name'];
  $acak           = rand(1,99);
  ...
  if (empty($lokasi_file)){
  mysqli_query($koneksi,"UPDATE sambutan SET judul       = '$_POST[judul]',
                                   judul_seo   = '$judul_seo', 
                                   isi_sambutan  = '$_POST[isi_sambutan]'  
                             WHERE id_sambutan   = '$_POST[id]'");
...
if(isset($_POST['kirimnewsletter']) && $_POST['kirimnewsletter']=='ya'){
  ...
  header('location:../../media.php?module='.$module.'&msg=update');
    ...
    $data_gambar = mysqli_query($koneksi,"SELECT gambar FROM sambutan WHERE id_sambutan='$_POST[id]'");
