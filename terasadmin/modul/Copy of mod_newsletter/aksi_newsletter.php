...
if ($module=='newsletter' AND $act=='hapus'){
  mysqli_query("DELETE FROM newsletter WHERE id='$_GET[id]'");
...
elseif($module=='newsletter' AND $act=='hapussemua'){
	...
	foreach($_POST['cek'] as $cek => $num){
		mysqli_query("DELETE FROM newsletter WHERE id=$num");
  header('location:../../media.php?module='.$module.'&msg=delete');
	}
	} else {
  header('location:../../media.php?module='.$module.'&msg=delete');
...
elseif ($module=='newsletter' AND $act=='input'){
  mysqli_query("INSERT INTO newsletter(email,username) VALUES('$_POST[email]','$_SESSION[namauser]')");
  
  mysqli_query("INSERT INTO newsletter(nama,username) VALUES('$_POST[nama]','$_POST[username]')");
...
elseif ($module=='newsletter' AND $act=='update'){
  mysqli_query("UPDATE newsletter SET email='$_POST[email]' WHERE id = '$id'");
