...
$pdf->addText(100, 820, 16,'<b>.:: Laporan Penjualan - GriyaParfum::.</b>');
...
$sql = mysqli_query($koneksi,"SELECT orders.id_orders as faktur,DATE_FORMAT(tgl_order, '%d-%m-%Y') as tanggal,
                    produk.nama_produk,jumlah,harga,diskon 
...
while($r = mysqli_fetch_array($sql)){
  ...
  $hargadisc = ($r[harga]-$disc);
  
  $quantityharga=rp($r[jumlah]*$hargadisc);
  
  $hargarp=rp($hargadisc); 
  $faktur=$r[faktur];
  ...
  $data[$i]=array('<b>No</b>'=>$i, 
	...
	$total = $total+($r[jumlah]*$hargadisc);
